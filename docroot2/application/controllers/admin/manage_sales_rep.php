...
class Manage_sales_rep extends CI_Controller {
    ...
    { 
        ...
        $base_url_address = ADMIN_FOLDER_NAME.'/manage_sales_rep/index/';

        //get and set pagination config variables
        $url_segment = 6;
        $limit = ADMIN_PAGING_LIMIT;
        $where = array('adm_status != ' => 'D','adm_usertype' =>'SR');
        ...
        $data['sal_rep_List'] = $this->mdgeneraldml->select('*', $tbl, $where, $order_by, $limit, $offset);       
         
        //count total number of sub-admins
        $sql4="SELECT count(*) as total_sal_rep_count FROM tbl_administrators WHERE adm_status !='D' AND adm_usertype = 'SR'";
        $numTotalSalesReps=$this->admin_model->sqlQuery($sql4);
        $data['total_sal_rep_count']=$numTotalSalesReps[0]['total_sal_rep_count'];
        
        //$data['userList']=$this->admin_model->getUserList();
        $this->load->view('admin/includes/header');	
        $this->load->view('admin/sales_rep_views/sales_rep_view',$data);
    ...
    {       $data=array(                   
            ...
            $this->form_validation->set_rules('sal_password', 'Password', 'trim|alpha_numeric|min_length[6]|max_length[12]|required|matches[csal_password]');
            ...
            {                
                $this->load->view('admin/includes/header');	
                $this->load->view('admin/sales_rep_views/sales_rep_add_view',$data);
                $this->load->view('admin/includes/footer');
            }
            else
            {       
               ...
               $this->session->set_flashdata('success', 'Sales Represetative has been successfully added.');
               redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep');
            }
    ...
    function edit($adm_id=NULL,$sort_by='adm_id', $sort_type='DESC', $offset=0)
    {
        if (($adm_id != NULL) && ($this->admin_model->isRecordExist('tbl_administrators', array('adm_id' => $adm_id)))) {
            ...
            $this->form_validation->set_rules('sal_password', 'Password', 'trim|min_length[6]|max_length[12]|alpha_numeric');
            $this->form_validation->set_rules('sal_contactno', 'Contact no', 'xss_clean|trim|is_numeric|required');
            ...
            if ($this->form_validation->run() == FALSE)
            {
                ...
                $this->load->view('admin/includes/header');	
                $this->load->view('admin/sales_rep_views/sales_rep_edit_view',$data);
                $this->load->view('admin/includes/footer');
            }
            else
            {
               ...
               $this->session->set_flashdata('success', 'Sales Representative has been successfully updated.');
               redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep');
            }
        }
        else
        {
            $this->session->set_flashdata('error', '!! Sales representative not exist.!!');
            redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep');
        }
    }
    ...
    function delete($adm_id=NULL, $sort_by='adm_id', $sort_type='DESC', $offset=0) 
    {
        if (($adm_id != NULL) && ($this->admin_model->isRecordExist('tbl_administrators', array('adm_id' => $adm_id)))) {
            ...
            $this->session->set_flashdata('success', 'Sales Representative has been Deleted successfully.');
            redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep/index/' . $sort_by . '/' . $sort_type . '/' . $offset);
        } else {
            $this->session->set_flashdata('error', '!! Sales Representative not exist.!!');
            redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep');
        }
    ...
    function soft_delete_selected($sort_by='adm_id', $sort_type='DESC', $offset=0)
    {
        ...
        {
            $sal_uids=implode(',',$_POST['chkmsg']);
            $this->admin_model->sqlUpdate("Update tbl_administrators SET adm_status='D',adm_updated_date='"._getDateAndTime()."' WHERE adm_id in($sal_uids)");
            $this->session->set_flashdata('success', 'Sales Representative has been Deleted successfully.');
            redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep/index/' . $sort_by . '/' . $sort_type . '/' . $offset);
        }
        else
        {
           $this->session->set_flashdata('error', 'Select atleast single sales representative to delete the record.');
           redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep'); 
        }
    }
    
    function deleted_salesreps($sort_by='adm_id', $sort_type='DESC', $offset=0)
    {
         # Pagination Starts Here -
        $base_url_address = ADMIN_FOLDER_NAME.'/manage_sales_rep/deleted_salesreps/';
        ...
        $tbl = 'tbl_administrators';
        $order_by = array('colname' => $sort_by, 'type' => $sort_type);
        $data['salesrepList'] = $this->mdgeneraldml->select('*', $tbl, $where, $order_by, $limit, $offset);

        //$data['userList']=$this->admin_model->getUserList();
        $this->load->view('admin/includes/header');	
        $this->load->view('admin/sales_rep_views/deleted_sales_rep_view',$data);
        $this->load->view('admin/includes/footer');
    }    
    ...
    function delete_from_system($adm_id)
    {
        if (($adm_id != NULL) && ($this->admin_model->isRecordExist('tbl_administrators', array('adm_id' => $adm_id)))) {
            ...
            $this->session->set_flashdata('success', 'Sales Representative has been Deleted successfully from the system.');
            redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep/deleted_salesreps/');
        } else {
            $this->session->set_flashdata('error', '!! Sales Representative not exist.!!');
            redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep/deleted_salesreps');
        }
   ...
   function hard_delete_selected($sort_by='adm_id', $sort_type='DESC', $offset=0)
    {
        ...
        {
            $sal_uids=implode(',',$_POST['chkmsg']);
            $this->admin_model->sqlDelete("DELETE FROM tbl_administrators WHERE adm_id in($sal_uids)");  
            $this->session->set_flashdata('success', 'Sales Representative has been Deleted successfully from the system.');
            redirect(ADMIN_FOLDER_NAME.'/manage_sales_rep/deleted_salesreps/' . $sort_by . '/' . $sort_type . '/' . $offset);           
        }
        else
        {
           $this->session->set_flashdata('error', 'Select atleast single sales representative to delete the record.');
