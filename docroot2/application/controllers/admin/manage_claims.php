...
function index($sort_by='priority', $sort_type='ASC', $offset=0) 
    {               
    ...
    function view($crId='',$sort_by='priority', $sort_type='ASC', $offset=0)
    {
    ...
    function save_satus($crId='',$sort_by='priority', $sort_type='ASC', $offset=0){
        $where=array('crStatus !='=>'Deleted','crId'=>$crId);
        if($crId!='' && _isRecordExist('tbl_cliam_requests',$where))
        {
            ...
            $priorityArray=array('Pending'=>1,'Approved'=>2,'Rejected'=>3,'Completed'=>4);
            $updateData=array('crStatus'=>$status,'priority'=>$priorityArray[$status],'crUpdatedOn'=>_getDateAndTime());
	...
	function delete_selected(){
		$crIds=implode(',',$_POST['chkmsg']);	
		if(!empty($_POST['chkmsg']))
        {
			$updateData=array('crStatus'=>'Deleted','crUpdatedOn'=>_getDateAndTime());
			$whereIn=array('column'=>'crId','fields'=>$_POST['chkmsg']);
            $this->mdgeneraldml->update_in('tbl_cliam_requests',$updateData,'', $whereIn);
            $this->session->set_flashdata('success', 'Selected Claims has been deleted successfully.');
            redirect(ADMIN_FOLDER_NAME.'/manage_claims');
        }
        else
        {
		   $this->session->set_flashdata('error', 'Select atleast single Claim to delete the record.');
           redirect(ADMIN_FOLDER_NAME.'/manage_claims'); 
        }
    }
}
