...
class Claim extends CI_Controller {
   ...
   function business($businessId=NULL)
    ...
    function registeration($type='basic', $step='1',$crId=NULL)
    {
        ...
        {
            //if user doesn't exist in the system then update claimrequiested user id =0
            if(!_isRecordExist('tbl_user',array('user_id'=>$claimRequestedUserId))){
                //tbl_cliam_requests crRequesterUserId crId
                $where123 = array('crId' => $claimRequestId);
                $update123 = array('crRequesterUserId' => 0);
                $this->mdgeneraldml->update($where123, 'tbl_cliam_requests', $update123);
                $claimRequestedUserId=0;
            }
