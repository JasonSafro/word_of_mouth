...
class Business_listing extends CI_Controller {
    ...
    function __construct() {
        ...
        _authenticateBusinessUser();//if the user is business user then only he can access this controller
    ...
    function add()
    {
        ...
        $data = array(
            ...
            'buss_license_docs'=>'',
            'buss_comp_email'=>'',
            'buss_comp_quot'=>0, //1 or 0
            'mon_d_and_s'=>0,//1 or 0
            'we_r_hir'=>0//1 or 0
        );
        
        
       $myTypeInfo=_getSingleRecord('user_plan,user_type,user_fname,user_lname,user_name','tbl_user',array('user_id'=>$userId));
       $data['my']=$myTypeInfo[0];
	   $my = $myTypeInfo[0];
		...
		if(($my['user_type']=='buss_user') && ($my['user_plan']=='pm' || $my['user_plan']=='pa')){
				$this->form_validation->set_rules('buss_comp_quot', 'competitive quotes', 'xss_clean|trim');
				if($this->input->post('buss_comp_quot')!="")
					$this->form_validation->set_rules('buss_comp_email', 'Email', 'xss_clean|trim|required|valid_email');
				else
					$this->form_validation->set_rules('buss_comp_email', 'Email', 'xss_clean|trim|valid_email');
				
				$this->form_validation->set_rules('mon_d_and_s', 'submit monthly deals & specials', 'xss_clean|trim');
				$this->form_validation->set_rules('we_r_hir', "We're Hiring", 'xss_clean|trim');
		}
        ...
        {  //print_r($_POST);DIE;
            $updateData = array(
                ...
                'buss_comp_email'=>$this->input->post('buss_comp_email'),
                'buss_comp_quot'=>($this->input->post('buss_comp_quot')!=""?1:0), //1 or 0
                'mon_d_and_s'=>($this->input->post('mon_d_and_s')!=""?1:0),//1 or 0
                'we_r_hir'=>($this->input->post('we_r_hir')!=""?1:0)//1 or 0
            );
            ...
            {    
               ...
               /* $mediaCoppyFileNames="";
                ...
                //send email to admin (new feature added by bhagwan dated on 20-May-2014)
                $businessUserName=$my['user_fname'].' '.$my['user_lname'];
                $businessUserName=($businessUserName!=""?$businessUserName:$my['user_name']);
                $businessName=$this->input->post('buss_name');
                $url=site_url('admin/manage_listing/edit/'.$businessId);
                $link='<a href="'.site_url($url).'">Click Here</a>';
                
                $where_Id=array('emailId'=>'120');
                ...
                $emailBody=str_replace ("[[USER_FULL_NAME]]", $businessUserName, $emilTemplet);
    ...
    function edit($businessId)
    {
        ...
        $data = array(
            ...
            'buss_comp_email'=>$info['buss_comp_email'],
            'buss_comp_quot'=>$info['buss_comp_quot'], //1 or 0
            'mon_d_and_s'=>$info['mon_d_and_s'],//1 or 0
            'we_r_hir'=>$info['we_r_hir']//1 or 0
            
        );
        
       $myTypeInfo=_getSingleRecord('user_plan,user_type','tbl_user',array('user_id'=>$userId));
       $data['my']=$myTypeInfo[0];
       $my=$myTypeInfo[0];
        ...
        if(!empty($_FILES['image_media'])){              
            if(!empty($_FILES['image_media']['name']) && $_FILES['image_media']['name'][0]!=""){              
                $this->form_validation->set_rules('image_media[]','Media Copy','callback_checkForMediaCopy['.$businessId.']');
            }
        }
        ...
        {  //print_r($_POST);DIE;
            ...
            if(($my['user_type']=='buss_user') && ($my['user_plan']=='pm' || $my['user_plan']=='pa')){
                $updateData['buss_comp_email']=$this->input->post('buss_comp_email');
                $updateData['buss_comp_quot']=($this->input->post('buss_comp_quot')!=""?1:0); //1 or 0
                $updateData['mon_d_and_s']=($this->input->post('mon_d_and_s')!=""?1:0);//1 or 0
                $updateData['we_r_hir']=($this->input->post('we_r_hir')!=""?1:0);//1 or 0
            }
    ...
    function checkForMediaCopy($str,$businessId=NULL)
    {        
        $userId=$this->session->userdata('user_id');
        $where = array('user_id' => $userId);
        $userInfo= $this->mdgeneraldml->select('user_plan,user_type', 'tbl_user', $where);
        $numFilesUlpoad=0;
        $imagesInDbForthisBusiness=0;
        ...
        if($businessId!=NULL){            
            $where = array('user_id' => $userId,'buss_id'=>$businessId);
        ...
        if(sizeof($files['name'])>$numFilesUlpoad){
            $this->form_validation->set_message('checkForMediaCopy', "You cannot upload more than $imageUploadeLimit image(s).");
            return FALSE;
        }else{
            $types=$files['type'];
            $flag=true;
