...
class Home extends CI_Controller {
    ...
    function search($limit=9){
        ...
        if($propertySearchText!="" && $propertySearchText!="Search for your provider"){
            if($bussIdsString!="")
                $whereSearchBusiness="(buss_name LIKE '%".$propertySearchText."%' OR buss_address LIKE '%".$propertySearchText."%' OR buss_city LIKE '%".$propertySearchText."%' OR buss_id in($bussIdsString) OR buss_description LIKE '%".$propertySearchText."%')";
            else
    ...
    public function business_details($bus_id=NULL,$id='')
    {
        ...
        {
            $userId=$this->session->userdata('user_id');
            $additionalInfo=$this->WGModel->sqlQuery("SELECT buss_comp_quot, mon_d_and_s, we_r_hir from tbl_business_info WHERE user_id=$userId");
            $myBuss_comp_quot=$additionalInfo[0]['buss_comp_quot'];
			$myMon_d_and_s=$additionalInfo[0]['mon_d_and_s'];
			$myWe_r_hir=$additionalInfo[0]['we_r_hir'];
        } */     
        
                    
            $additionalInfo=$this->WGModel->sqlQuery("SELECT buss_comp_quot, mon_d_and_s, we_r_hir from tbl_business_info WHERE buss_id=$bus_id");
        ...
        $data['buss_comp_quot']=$myBuss_comp_quot;
		$data['mon_d_and_s']=$myMon_d_and_s;
		$data['we_r_hir']=$myWe_r_hir;
    ...
    function __getBusinessDetails($bus_id){
        ...
        //get deals
        $whereDeals="WHERE dealBusinessId=$bus_id AND dealStatus !='Deleted'";
        $data['dealList'] = $this->WGModel->getDealList($whereDeals);
        
        //get jobs
        //$whereJobs="WHERE jobBusinessId=$bus_id AND jobStatus!='Deleted'  AND jobPostDate >=NOW()";
    ...
    function add_business_review($bus_id,$id=''){    
        ...
        if($this->form_validation->run() == FALSE){            
            ...
            $reviewText=array('5'=>'Excellent','4'=>'Good','3'=>'Average','2'=>'Poor','1'=>'Awful');
            //send Email
            ...
            $emailBody=str_replace ("[[RATE_PROF]]", $reviewText[$rate_pro], $emailBody);
            $emailBody=str_replace ("[[RATE_DEPEND]]", $reviewText[$rate_depend], $emailBody);
            $emailBody=str_replace ("[[RATE_PRICE]]", $reviewText[$rate_price], $emailBody);
            $emailBody=str_replace ("[[RATE_OVERALL]]", $reviewText[$rate_overall], $emailBody);
   ...
   function rvwDetailsChk($str){
    ...
    function __getJobInfo($jobId){
        ...
        $whereJobs="WHERE buss_id IN(SELECT buss_id FROM tbl_business_categories WHERE cat_id IN(SELECT cat_id FROM tbl_business_categories WHERE buss_id=$businessId)) AND jobStatus!='Deleted'   AND jobPostDate >=NOW()";
    ...
    function competitive_quotes($business_id)
    {
        ...
        $this->form_validation->set_rules('phno', 'Phone', 'xss_clean|trim|required|numeric');
        $this->form_validation->set_rules('email', 'Email', 'xss_clean|trim|required|valid_email');
        $this->form_validation->set_rules('msg_quotes', 'Message Quote', 'xss_clean|trim|required');
        ...
        else
        {
           ...
           $compEmail=$data['info']['buss_comp_email'];
           $bussEmail=$data['info']['buss_email'];
           $comp=$data['info']['buss_comp_quot'];
           $buss_email=($comp==1?($compEmail!=""?$compEmail:$bussEmail):$bussEmail);           
	   $buss_name=$data['info']['buss_name'];
		   //$buss_web_address=$data['info']['buss_web_address'];
           ...
           $emailinfo=$this->mdgeneraldml->select('*','tbl_email_contents',$where_Id);
            ...
            $emailBody=str_replace ("[[BUSS_NAME]]", $buss_name, $emailBody);
            
            //$mail = @send_bcc_email($email_bcc,$buss_email,$emilTempletSubject,$emailBody,$email_bcc);
            //sent to admin
            @send_email(ADMIN_EMAIL,$emilTempletSubject,$emailBody);
            //sent to business user
            @send_email($buss_email,$emilTempletSubject,$emailBody);
			
            $this->session->set_flashdata('success','Quote request sent successfully.');
            $url = site_url("home/business_details/$business_id#success");
            $successString = "Quote request sent successfully.";
