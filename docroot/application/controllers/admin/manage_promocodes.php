...
class Manage_promocodes extends CI_Controller {
	...
	function delete_selected($sort_by='pc_id', $sort_type='DESC', $offset=0){
        
		if(!empty($_POST['chkmsg']))
        {

			$promoIds=implode(',',$_POST['chkmsg']);	
			
            //echo $bussIds; die;
            $whereIn=array('column'=>'pc_id','fields'=>$_POST['chkmsg']);
            $this->mdgeneraldml->delete_in('tbl_promo_codes', $whereIn);
            $this->session->set_flashdata('success', 'Selected Promocodes has been deleted successfully.');
            redirect(ADMIN_FOLDER_NAME.'/manage_promocodes/index/' . $sort_by . '/' . $sort_type . '/' . $offset);           
        }
        else
        {
		   $this->session->set_flashdata('error', 'Select atleast single promocode to delete the record.');
           redirect(ADMIN_FOLDER_NAME.'/manage_promocodes/index/' . $sort_by . '/' . $sort_type . '/' . $offset); 
        }
    }
