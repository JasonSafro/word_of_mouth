...
class Reasons extends CI_Controller {
	...
	function delete_selected($sort_by='reason_id', $sort_type='DESC', $offset=0){
		...
		if(!empty($_POST['chkmsg']))
        {
            ...
            //echo $bussIds; die;
            $whereIn=array('column'=>'reason_id','fields'=>$_POST['chkmsg']);
            $this->mdgeneraldml->delete_in('tbl_contact_reasons', $whereIn);
            $this->session->set_flashdata('success', 'Selected reasons has been deleted successfully.');
            redirect(ADMIN_FOLDER_NAME.'/reasons/index/' . $sort_by . '/' . $sort_type . '/' . $offset);           
        }
        else
        {
		   $this->session->set_flashdata('error', 'Select atleast single reason to delete the record.');
           redirect(ADMIN_FOLDER_NAME.'/reasons/index/' . $sort_by . '/' . $sort_type . '/' . $offset); 
        }
    }
}
