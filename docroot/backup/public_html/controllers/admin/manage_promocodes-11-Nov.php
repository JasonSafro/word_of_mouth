...
function index($sort_by='pc_id', $sort_type='Desc', $offset=0) 
    {             
        ...
        $data = _getPagingVaiables('tbl_promo_codes', $url_segment, $base_url_address, $limit, $offset, $sort_by, $sort_type);   
    ...
    function addPromocodes(){
		if(isset($_POST['submit']) && !empty($_POST['submit'])){
            ...
            {               
				...
				$insertData=array(
                        'pc_plan_type_id'         =>$this->input->post('plan_type'), 
                        'pc_plan_id'     =>$this->input->post('plan_name'),                       
			...
			$data['plan_types'] = $this->mdgeneraldml->select("subs_sub_plan_id,subs_sub_plan_name","tbl_subscription_sub_plans",array("subs_sub_plan_status"=> 'A')); 
			$data['action'] = 'add';
			$this->load->view('admin/includes/header');	
	...
	function EditPromocodes($promo_id=NULL)
    {
		...
		if(isset($_POST['submit']) && !empty($_POST['submit'])){
			...
			$data = array(
						...
						'pc_id'       => $promo_id,
                        'plan_type'       => $rec[0]['pc_plan_type_id'],
